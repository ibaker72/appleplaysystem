...
export default async function HomePage() {
  const features = await getFeatures({ limit: 6 });
  ...
  return (
    <>
      ...
      <PremiumSection eyebrow="Process" title="How it works">
        <div className="grid gap-3 text-sm text-white/70 md:grid-cols-4">
          {["Check compatibility", "Select features", "Secure checkout", "Book remote session"].map((step, i) => (
            <div className="surface rounded-xl p-4" key={step}>
              <p className="text-white/40">0{i + 1}</p>
      ...
      <PremiumSection eyebrow="Questions" title="FAQ">
        <FAQAccordion />
      </PremiumSection>
      <PremiumCTA />
    </>
